    ...
    return `
      <div class="settings-store-item">
        ...
        <div class="settings-store-actions">
          <button class="btn-icon btn-whitelist ${isWhitelisted ? 'active' : ''}" title="${isWhitelisted ? 'Nonaktifkan perlindungan hibernasi' : 'Lindungi dari hibernasi otomatis'}" aria-label="${isWhitelisted ? 'Nonaktifkan' : 'Aktifkan'} perlindungan hibernasi untuk ${escapeHtml(store.name)}" onclick="toggleWhitelist('${store.id}')">
          ...
          <button class="btn-icon" title="Edit" aria-label="Edit toko ${escapeHtml(store.name)}" onclick="openEditModal('${store.id}')">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
              <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
              <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
            </svg>
          </button>
          <button class="btn-icon danger" title="Hapus" aria-label="Hapus toko ${escapeHtml(store.name)}" onclick="deleteStore('${store.id}')">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
